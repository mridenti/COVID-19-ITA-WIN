...
# Cenario folder
cenario_folder = 'cenarioFortaleza'
...
def f(x):
    ...
    g_e = x[2]
    print(r_0, g_s, g_e)
    subprocess.call(['python', 'cenario_generator.py', '-i', 'cenarioFortaleza', '-d', '0', '7', '94', '219', '-m', '3',
...
def f_s(x, params):
    ...
    g_e = params[1]
    fat_fac = params[2]
    subprocess.call(['python', 'cenario_generator.py', '-i', cenario_folder, '-d', '0', '7', '94', '219', '-m', '3',
                     '-I0', str(8.0*g_s), '-R0', str(r_0), '-Rp', str(r_0), '-epi', str(g_e),
...
subprocess.call(['python', 'cenario_generator.py', '-i', cenario_folder, '-d', '0', '7', '94', '219', '-m', '3',
                 '-I0', str(8*ret.x[1]), '-R0', str(ret.x[0]), '-Rp', str(ret.x[0]), '-epi', str(ret.x[2]),
                 '-itv', '0', '10', '9', '1', '-s'])
...
subprocess.call(['bin/spatial_covid0d_estrat.exe', 'input/generated-input.txt', '/'.join(['output', output_file]),
                 '3'])
os.chdir("scripts")
subprocess.call(['python', 'plot_output_SEAHIR_Fortaleza.py', '-d', '0', '7', '94', '219', '-s', str(ret.x[1])])

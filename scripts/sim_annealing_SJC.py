...
def f(x):
    ...
    g_e = x[2]
    print(r_0, g_s, g_e)
    subprocess.call(['python', 'cenario_generator.py', '-i', cenario_folder, '-d', '0', '21', '96', '221', '-m', '3',
...
subprocess.call(['bin/spatial_covid0d_estrat.exe', 'input/generated-input.txt', '/'.join(['output', output_file]),
                 '3'], stdout=open(os.devnull, 'wb'))
os.chdir("scripts")
subprocess.call(['python', 'plot_output_SEAHIR_SJC.py', '-d', '0', '21', '96', '221', '-s', str(ret.x[1])])